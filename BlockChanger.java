...
public class BlockChanger {
	...
	static {
		...
		BlockUpdater blockUpdater = null;
		...
		if (ReflectionUtils.supports(18)) {
			// 18 and up
			blockUpdater = new BlockUpdaterLatest(BLOCK_NOTIFY, CHUNK_SET_TYPE, GET_SECTION_INDEX, GET_LEVEL_HEIGHT_ACCESSOR);
		} else if (ReflectionUtils.supports(17)) {
			// 17 only
			blockUpdater = new BlockUpdater17(BLOCK_NOTIFY, CHUNK_SET_TYPE, GET_SECTION_INDEX, CHUNK_SECTION, SET_SECTION_ELEMENT);
		} else if (ReflectionUtils.supports(13)) {
			// 1.13 - 1.16
			blockUpdater = new BlockUpdater13(BLOCK_NOTIFY, CHUNK_SET_TYPE, CHUNK_SECTION, SET_SECTION_ELEMENT);
		} else if (ReflectionUtils.supports(9)) {
			// 1.9 only
			blockUpdater = new BlockUpdater9(BLOCK_NOTIFY, CHUNK_SET_TYPE, CHUNK_SECTION, SET_SECTION_ELEMENT);
		} else if (ReflectionUtils.supports(8)) {
			// 1.8 only
			blockUpdater = new BlockUpdaterLegacy(BLOCK_NOTIFY, CHUNK_SET_TYPE, CHUNK_SECTION, SET_SECTION_ELEMENT);
		} else {
			// 1.8-
			blockUpdater = new BlockUpdaterAncient(BLOCK_NOTIFY, CHUNK_SET_TYPE, CHUNK_SECTION, SET_SECTION_ELEMENT);
		}

		BLOCK_UPDATER = blockUpdater;
		...
		Bukkit.getWorlds().forEach(world -> NMS_WORLDS.put(world, getNMSWorld(world)));
	 ...
	 * to another location as if using the vanilla command /fill
	 * @param loc1 point 1
	 * @param loc2 point 2
	 * @param material material to apply on the blocks
	 */
	 ...
	 */
	private static Object newBlockPosition(@Nullable Object world, Object x, Object y, Object z) {
		try {
	...
	/**
	 * @return all available block materials for the current version separated by commas as follows: <i>dirt, stone, glass, etc...</i>
	 */
